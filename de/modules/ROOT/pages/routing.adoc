...
Eine Route zwischen einem Maschinen- und einem Softwareendpunkt werden automatisch erstellt. Dabei kann ein Maschinenendpunkt zu beliebig vielen Softwareendpunkten eine Route aufbauen sowie umgekehrt. 

[TIP]
Falls keine automatische Route erstellt wird, ist die Funktion für automatische Verbindungen am Endpunkt deaktiviert. Füge dafür zunächst die manuelle Route zu deinem gewünschten Endpunkt hinzu. Öffne anschließend die Endpunktdetails indem du auf den Endpunkt klickst, aktiviere die Funktion *_Automatische Verbindung zwischen Endpunkten_*, und künftige Verbindungen werden automatisch hergestellt.
