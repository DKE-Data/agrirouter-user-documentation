...
Dein KUHN Anbaugerät muss mit dem agrirouter verbunden sein, um drahtlos Telemetriedaten an dein Farm-Management-System und die myKUHN Plattform senden zu können. In diesem Abschnitt erfährst du, wie du 
...
. Nachdem sowohl dein KUHN EasyTransfer, als auch dein Terminal mit dem agrirouter verbunden sind, muss noch eine Route zwischen den beiden Endpunkten innerhalb deines agrirouter-Kontos erstellt werden. Wie das funktioniert, erfährst du link:https://manual.agrirouter.com/de/routing.html[hier, window="_blank"].
